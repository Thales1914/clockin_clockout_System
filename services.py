...
def init_db():
    with get_db_connection() as conn:
        ...
        if cursor.fetchone()[0] == 0:
            initial_users = [
                ('admin', 'Administrador', 'Sistema', _hash_senha('admin123'), 'admin'),
                ('101', 'Carlos', 'RH', _hash_senha('123'), 'employee'),
                ('102', 'Eduardo', 'RH', _hash_senha('456'), 'employee')
            ]
            cursor.executemany("INSERT INTO funcionarios (codigo, nome, cargo, senha, role) VALUES (?, ?, ?, ?, ?)", initial_users)
...
def ler_funcionarios_df():
    with get_db_connection() as conn:
        df = pd.read_sql_query("SELECT codigo, nome, cargo, role FROM funcionarios", conn)
    return df
...
def adicionar_funcionario(codigo, nome, cargo, senha):
    if not all([codigo, nome, cargo, senha]):
    ...
    try:
        with get_db_connection() as conn:
            with conn:
                ...
                senha_hash = _hash_senha(senha)
                cursor.execute(
                    "INSERT INTO funcionarios (codigo, nome, cargo, senha, role) VALUES (?, ?, ?, ?, ?)",
                    (codigo, nome, cargo, senha_hash, 'employee')
                )
    except sqlite3.Error as e:
...
def gerar_relatorio_organizado_df(df_registros: pd.DataFrame) -> pd.DataFrame:
    ...
    dt_inicio_expediente = pd.to_datetime(df_final['Data'] + ' ' + df_final['Início do Expediente'].astype(str), errors='coerce')
    dt_fim_expediente = pd.to_datetime(df_final['Data'] + ' ' + df_final['Fim do Expediente'].astype(str), errors='coerce')
    dt_inicio_almoco = pd.to_datetime(df_final['Data'] + ' ' + df_final['Início do Almoço'].astype(str), errors='coerce')
    dt_fim_almoco = pd.to_datetime(df_final['Data'] + ' ' + df_final['Fim do Almoço'].astype(str), errors='coerce')
